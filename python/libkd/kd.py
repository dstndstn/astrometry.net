...
def _buildtrees(x1, x2):
    ...
    kd1 = _libkd.kdtree_build(fx1)
    if fx2 is fx1:
        kd2 = kd1
    else:
        kd2 = _libkd.kdtree_build(fx2)
    return (kd1, kd2)

def _freetrees(kd1, kd2):
    _libkd.kdtree_free(kd1)
    if kd2 != kd1:
        _libkd.kdtree_free(kd2)
...
def match(x1, x2, radius, notself=False, permuted=True, indexlist=False):
    ...
    if indexlist:
        inds = _libkd.match2(kd1, kd2, radius, notself, permuted)
    else:
        (inds,dists) = _libkd.match(kd1, kd2, radius, notself, permuted)
    _freetrees(kd1, kd2)
    if indexlist:
        return inds
    return (inds,dists)
...
def nearest(x1, x2, maxradius, notself=False, count=False):
    ...
    (kd1,kd2) = _buildtrees(x1, x2)
    if count:
        X = _libkd.nearest2(kd1, kd2, maxradius, notself, count)
    else:
        X = _libkd.nearest(kd1, kd2, maxradius, notself)
    _freetrees(kd1, kd2)
    return X
...
def tree_build(X):
    ...
    '''
    return _libkd.kdtree_build(X)

def tree_free(kd):
    _libkd.kdtree_free(kd)

def tree_save(kd, fn):
    rtn = _libkd.kdtree_write(kd, fn)
    return rtn

def tree_open(fn, treename=None):
    if treename is None:
        return _libkd.kdtree_open(fn)
    else:
        return _libkd.kdtree_open(fn, treename)

def tree_close(kd):
    return _libkd.kdtree_close(kd)

def tree_search(kd, pos, radius, getdists=False, sortdists=False):
    return _libkd.kdtree_rangesearch(kd, pos, radius,
                                            int(getdists), int(sortdists))

tree_permute = _libkd.kdtree_permute
tree_bbox = _libkd.kdtree_bbox
tree_n = _libkd.kdtree_n
tree_print = _libkd.kdtree_print
tree_data = _libkd.kdtree_get_positions
